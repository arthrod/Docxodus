...
**Comment fidelity.** Comments (`w:commentRangeStart`/`End`/`w:commentReference`) are dropped from IR paragraphs (reader rule N15 records them as `IrCommentStore` char-offset spans for the markdown projection only), so the fine token-diff path would rebuild an edited commented paragraph without them and orphan the `comments.xml` definition. An edited paragraph carrying comment markers therefore bails to the conservative whole-block `del(left)+ins(right)` fallback (`ParagraphCarriesComments`), cloning the raw paragraphs so the markers survive; a post-render `DeduplicateRevisionDuplicatedCommentIds` pass gives the deleted copy a fresh comment id + a cloned definition so the duplicated del/ins ids are schema-valid, and accept/reject each resolve to one comment. (The blessed `WmlComparer` oracle drops comments entirely on any edit; carrying comment markers through the token diff for fine-grained comment-edit markup is a future enhancement.)
