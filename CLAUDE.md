...
- Tier C (formatting): `ApplyFormat(anchor, CharSpan?, FormatOp)` (`FormatOp.FontSizePts` → `w:sz`/`w:szCs`; `FormatOp.FontFamily` → `w:rFonts` ascii/hAnsi/cs, `""` clears), `SetParagraphStyle(anchor, styleId)`, `SetParagraphFormat(anchor, ParagraphFormatOp)` (alignment/page-break + indent: `IndentDelta` (left ±) / `LeftIndent` (absolute) / `FirstLineIndent` (signed: >0 first-line, <0 hanging, 0 clears — mutually-exclusive `w:firstLine`/`w:hanging`) → `w:ind` + `TopBorder`/`BottomBorder`/`ClearBorders` → `w:pBdr`), `SetListLevel(anchor, delta)`, `RemoveListMembership(anchor)`, `ApplyMultilevelNumbering(anchor, levels, level, restart)` (caller-configurable multi-level legal numbering — each `NumberingLevel` carries `Format`/`LevelText` (e.g. `"%1."`/`"(%2)"`)/indent; `NumberingFactory.EnsureMultilevel` dedups identical schemes by a hashed `w:nsid` so they continue one sequence, `restart` mints a fresh one; the paragraph becomes an `li`, so `SetListLevel`/Tab handle levels)
